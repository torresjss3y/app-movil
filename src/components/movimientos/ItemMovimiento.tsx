...
import { Colors, ListItemHeight, Spacing } from "@/constants/theme";
...
export function ItemMovimiento({ lote, onPress }: Props) {
  ...
  // Si el lote proviene de una venta, mostramos su monto en lugar de la cantidad.
  const esVenta = lote.total_venta !== null && lote.total_venta !== undefined;

  const cantidadTexto = `${signo}${lote.total_unidades} u.`;
    ...
    <TouchableOpacity activeOpacity={0.7} onPress={onPress} accessibilityRole="button" accessibilityLabel={`${motivoTexto} · ${lote.total_unidades} unidades · ${formatearFechaRelativa(lote.fecha)}`}>
      <ThemedView style={[styles.card, { borderColor: theme.border, backgroundColor: theme.card }]}>
        ...
        <View style={styles.content}>
          ...
          <ThemedText type="small" numberOfLines={1} style={[styles.meta, { color: theme.textTertiary }]}>
            {etiquetaProductos} · {formatearFechaRelativa(lote.fecha)}
          </ThemedText>
        </View>

        {/* Monto (ventas) o cantidad (entradas/salidas/ajustes) a la derecha */}
        <ThemedText type="smallBold" style={[styles.valor, { color: esVenta ? theme.success : colorTipo }]}>
          {esVenta ? `S/ ${(lote.total_venta as number).toFixed(2)}` : cantidadTexto}
        </ThemedText>
      </ThemedView>
    </TouchableOpacity>
  );
